...
(async () => {
  ...
  try {
    ...
    const token = `${Buffer.from('{"alg":"HS256","typ":"JWT"}').toString('base64url')}.${Buffer.from('{"sub":"42"}').toString('base64url')}.sig`;
    await post({ type: 'load', text: token, name: 'token.jwt' }); await waitText('#input-status', 'Invalid JSON');
    await action('input', 'JWT'); await waitText('#input-status', 'Valid JSON'); await page.waitForTimeout(700); assert.match(await copy('output'), /"alg": "HS256"[\s\S]*"sub": "42"[\s\S]*"signature": "sig"/); checks.push('JWT decode');
    const paste = text => page.evaluate(t => { const area = document.querySelector('#input-editor textarea'); area.focus(); const data = new DataTransfer(); data.setData('text/plain', t); area.dispatchEvent(new ClipboardEvent('paste', { clipboardData: data, bubbles: true, cancelable: true })); }, text);
    await post({ type: 'load', text: '', name: 'paste.json' }); await waitText('#input-status', 'Paste JSON to begin');
    await paste(token); await waitText('#input-status', 'Valid JSON'); await page.waitForTimeout(700); assert.match(await copy('input'), /"alg": "HS256"[\s\S]*"signature": "sig"/);
    await post({ type: 'load', text: '', name: 'paste.json' }); await waitText('#input-status', 'Paste JSON to begin');
    await paste('OK'); await waitText('#input-status', 'Invalid JSON'); await page.waitForTimeout(700); assert.equal(await copy('input'), 'OK'); checks.push('paste auto-decodes a JWT and leaves plain text alone');
